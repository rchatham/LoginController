...
public class VerificationVC : ESModalViewController, UITextFieldDelegate  {
    ...
    var existingUserCheck: ((RCUser, (Bool, RCUser) -> Void ) -> Void)?
        ...
        if smsCodeTextField.text != nil && !smsCodeTextField.text!.isEmpty {
            loginController.didEnter(token: smsCodeTextField.text!, openingHandler: {
                ...
                if self?.view.center != nil {
                    ...
                    // Check if RCUser already exists on server or if needs to be created
    ...
    func shouldCreateUser(user: RCUser) {
